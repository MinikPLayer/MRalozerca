...
int vibrate(float time)
{
    cout << "Vibration disabled!!!" << endl;
}
#endif // !USE_JNI
...
const float powerUpsFallSpeed = 1.0;
const int powerUpsSize = 4;
const int powerUpSpawnChance = 10; // Szansa 1 na x w ci¹gu sekundy
...
public:
    ...
    PowerUpDrawable(powerUpTypes stType, Vector2f stPos, Texture stTexture);
    PowerUpDrawable(powerUpTypes stType, Vector2f stPos, string stTexturePath);
    PowerUpDrawable(powerUpTypes stType, Vector2f stPos);
...
void PowerUpDrawable::move(Vector2f move)
{
    pos += move;
    sprite.setPosition(pos);
}

void PowerUpDrawable::move(float moveX, float moveY)
{
    pos.x += moveX;
    pos.y += moveY;
...
void PowerUpDrawable::setPosition(Vector2f newPos)
{
    pos = newPos;
...
void PowerUpDrawable::setPosition(float newPosX, float newPosY)
{
    pos.x = newPosX;
    pos.y = newPosY;
...
PowerUpDrawable::PowerUpDrawable(powerUpTypes stType, Vector2f stPos, Texture stTexture)
{
    type = stType;
    pos = stPos;
    texture = stTexture;
...
PowerUpDrawable::PowerUpDrawable(powerUpTypes stType, Vector2f stPos, string stTexturePath)
{
    type = stType;
    pos = stPos;
    texture.loadFromFile(stTexturePath);

    sprite.setTexture(texture);
}

PowerUpDrawable::PowerUpDrawable(powerUpTypes stType, Vector2f stPos)
{
    type = stType;
    pos = stPos;
}
...
powerUpTypes actualFallingPowerUpType;

vector<PowerUpDrawable> droppedPowerUps;

void ManagerPowerUpsMove(float DeltaTime)
{
    for (int i = 0; i < droppedPowerUps.size(); i++)
    {
        droppedPowerUps[i].move(0, DeltaTime * gameSpeed);
    }
}

void PowerUpsSpawnHander()
{
    if (PowerUpSpawned)
    {
        PowerUpSpawnHandlerTimer = Time::Zero;
        return;
    }
    PowerUpSpawnHandlerTimer += PowerUpSpawnHandlerClock.restart();
    if (PowerUpSpawnHandlerTimer.asSeconds() >= 1)
    {
        PowerUpSpawnHandlerTimer = Time::Zero;
        int randNmbr = rand() % powerUpSpawnChance;
        if (randNmbr == 1)
        {
            PowerUpSpawned = true;
            //PowerUpX = rand() % 1920;
            //PowerUpY = 0 - rand() % 200;
            powerUpTypes spawnedType = (powerUpTypes)(rand() % powerUpsSize);
            droppedPowerUps.push_back(PowerUpDrawable(spawnedType, Vector2f(rand() % 1750 + 50, 0 - rand() % 200), "images/powerUp (" + to_string( (int)spawnedType ) + ").png"));
            //actualFallingPowerUpType = spawnedType;
        }
    }
}

bool SaveActualState(float MrX, long int punkty, long int zycia, float marchX, float marchY, short MRkolor, short mchkolor, bool stworzona, float predkoscmarchewki)
{
    ofstream plik;
...
bool loadActualSaveState(float & MrX, long int & punkty, long int & zycia, float & marchX, float & marchY, short & MRkolor, short & mchkolor, bool & stworzona, float & predkoscmarchewki)
{
    string firstLine;
    ifstream plik;
    plik.open(actualSaveFileDirectory);
...
int main()
{
    ...
    short ciag[2] = { 0,0 };	// Która z rzêdu zjedzona marchewka
                                //long int punkty[2] = { 0,0 };	// Iloœæ punktów
   ...
   // int resY = VideoMode::getDesktopMode().height;		//Rozdzielczoœæ ekranu Y - domyœlna wartoœæ
    ...
    float glosnosc = 100;	// G³oœnoœæ g³ówna gry
    ...
    float animframetime = 50; // Parametr okreœla aktualn¹ prêdkoœæ zmiany klatki animacji ( w milisekundach )
    float animframetimemove = 50; // Parametr okreœla domyœlny co jaki czas zmieniana bêdzie klatka animacji w ruchu ( w milisekundach )
    float animframetimestay = 250; // Parametr okreœla domyœlny co jaki czas zmieniana bêdzie klatka animacji w spoczynku ( w milisekundach )

    float FishAnimationTime = 600; // Parametr okreœla jak d³uga ma byæ obrót rybki ( w milisekundach )

    bool wRuchu = 0; // Okreœla czy Grunio siê porusza
    ...
    Sprite PowerUps_spr[powerUpsSize];
    ...
    Texture txt_powerUps[powerUpsSize];

    Grunio grunio[2];
    grunio[0].x = resX / 2;
    grunio[0].y = resY - 220; // 220 before christmas update
    grunio[1].x = resX / 2;
    grunio[1].y = resY - 220;
    ...
    for (int i = 0; i < powerUpsSize; i++)
    {
        string cache_directory_powerups = "images/powerUp (" + to_string(i) + ").png";
        txt_powerUps[i].loadFromFile(cache_directory_powerups);
    }
    ...
    for (int i = 0; i < powerUpsSize; i++)
    {
        PowerUps_spr[i].setTexture(txt_powerUps[i]);
    }
    ...
    case 2:
    {
        //MessageBox(NULL, "Nie uda³o siê wczytaæ czcionki!\nUpewnij siê, ¿e plik gabriola.ttf jest w g³ównym folderze gry.", "Blad wczytywania czcionki!", MB_OK | MB_ICONERROR);
        return 3;
        break;
    }
    case 3:
	...
	if (maxscore > 0)
	{
		...
		if (maxscore == 29)
		{
			bestscore.setString("Autor gry: Michal Tomecki.\n Podziekowania dla: Agnieszki ( pomysl i projekt grafik ) oraz Zuzy ( testy i pomysly )");
		}
	}
    ...
    short grsizeX = 169;		//Szerokoœæ tekstury grunia w px
    short grsizeY = 142;		//Wysokoœæ tekstury grunia w px ( 111 before christmas update )
    short marchsizeX = 74;		//Szerokoœæ tekstury marchewki w px
    short marchsizeY = 54;		//Wysokoœæ tekstury marchewki w px
	...
	*/
    ...
    if (lastExitMode == 2) loadActualSaveState(grunio[0].x, grunio[0].punkty, grunio[0].zycia, marchX[0], marchY[0], grunio[0].kolor, mchkolor[0], stworzona[0], predkoscmarchewki[0]);
    ...
    while (okno.isOpen())
    {
        ...
        ManagerPowerUpsMove(DeltaTime);

        actualPowerUps.managerPowerUpsTimers();
        //widok = okno.getView();
        for (int i = 0; i < 5; i++)
        {
        ...
        if (okno.pollEvent(zdarzenie) && active)
        {
			...
			if (zdarzenie.type == sf::Event::LostFocus)
			{
				...
				//active = false;
                SaveActualState(grunio[0].x, grunio[0].punkty, grunio[0].zycia, marchX[0], marchY[0], grunio[0].kolor, mchkolor[0], stworzona[0], predkoscmarchewki[0]);
                okno.close();
                exit(1);
                return 1;
				break;
		...
		if (active)
		{
            ...
            for (int i = 0; i < droppedPowerUps.size(); i++)
            {
                okno.draw(droppedPowerUps[i].sprite);
            }
			okno.draw(gabeczka);
